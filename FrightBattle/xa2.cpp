...
//X3DAudioInitialize(XAUDIO2_DEFAULT_CHANNELS, X3DAUDIO_SPEED_OF_SOUND, x3DInstance);
...
{
	{"asset/BGM/Title.wav", true},			// タイトルBGM（ループさせるのでtrue設定）
	{"asset/BGM/Stage_01.wav", true},		// ステージBGM（ループさせるのでtrue設定）
	{"asset/BGM/Title.wav", true},			// リザルトBGM（ループさせるのでtrue設定）
	{"asset/SE/StartGame.wav", false},		// ミサイルを撃った時のSE	
	{"asset/SE/MoveTitle.wav", false},		// ミサイルを撃った時のSE
	{"asset/SE/MisileShotto.wav", false},	// ミサイルを撃った時のSE
	{"asset/SE/MisileHit.wav", false},		// ミサイルが当たった時のSE
	{"asset/SE/LockOn.wav", false},			// ロックオン時のSE
	{"asset/SE/EnemyRock.wav", false},		// 敵にロックされたとき
	{"asset/SE/Buzer.wav", true},			// 敵をロックしたとき
	{"asset/SE/CountDown.wav", false},		// カウントダウン
	{"asset/SE/CountDown2.wav", false},		// カウントダウン2
};
...
void XA_Play(SOUND_LABEL label , const float& _volume , const float& _pitch)
{
	...
	// 音量を 0.0f (ミュート) 〜 1.0f (フルボリューム) に設定
	pSV->SetVolume(_volume);

	//// 再生速度を 0.5 (50%) 〜 2.0 (200%) に設定
